...
curl --retry 15 -f --retry-all-errors --retry-delay 1 -s "http://localhost:26657/block?height=1" | \
  jq '.error == null' && \
  echo "Node started" || {
...
archwayd q staking delegations-to "${VALIDATOR_ADDRESS}" | jq .
